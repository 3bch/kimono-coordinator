- [x] 各アイテムは初期に単色のものを事前登録しておき、登録0にはできない案を検討する
    - 採らない。事前登録した単色は「所有している 1 点」ではないので削除され、
      結局 0 点に戻る。防ぐには「最後の 1 点は削除できない」制約が要り、
      モードレス方針とも噛み合わない
    - 代わりに 0 点の見せ方を決めた。ゴーストを出すのは「無いと困るのに無い」
      ときだけで、羽織・帯留めは 0 点でも「なし」として何も描かない
      （詳細は `docs/wireframes.md`）
- [x] oxlint のルール設定
    - [x] もうちょっと見直し
    - [x] type aware 検討
- [x] boundary が使えるか確認 → 使えなかった
- [x] TanStack Router 設定
- [x] vitest 設定
    - [x] できたけど遅い問題
- [x] npm をふさぐ
- [x] CLAUDE.md や カスタムコマンド系 の設定
- [x] Vite 8 を試す
- [x] shadcn を BaseUI ベースに移行する
- [x] playwright-cli でブラウザ確認をエージェントができるようにする
- [x] 背景透過に関する方針を決める → `@imgly/background-removal` に決定
    - 方針: クライアントサイド推論。SaaS API はバックエンドと月額費用が要るので見送り
    - `@imgly/background-removal` を帯留め画像で検証: 外形はきれいに抜けるが、
      透かし(穴)部分に背景が残る。処理時間は devcontainer の WASM で 20〜30 秒
    - ライセンスは AGPL-3.0 なので、非 OSS で商用化する場合は要商用ライセンス
    - `transformers.js` + BiRefNet_lite も試したが、ブラウザの WASM では
      fp32 / fp16 とも std::bad_alloc(メモリ不足)で断念(モデルは MIT で商用可)
    - 検証ページは `/bg-test`(検証用画像は `public/obidome-sample.jpg`)
- [ ] まずは UI の試作
    - [x] スワイプ動作
    - [x] 着物の SVG
    - [x] 着物と帯のスワイプ判定
    - [x] スワイプで戻るときに隣がすぐに消える問題
    - [x] 帯締を追加する
    - [x] react-swipeable への移行を検討する -> 移行しないとなった
    - [x] 寸胴なのでシルエットをすっきりさせる
    - [x] おびあげを追加する
    - [x] 帯留めを追加する
    - [x] 羽織を追加する
    - [x] 半襟を追加する
    - [x] 帯締めを狭くする
    - [x] 首まわりがキツいので緩和する
    - [x] 肩のところに丸みをつける
    - [x] 衿の線
    - [x] 白い部分の線をきれいに引く
        - [x] SVG を更新する
        - [x] tsx に反映する
        - [x] 帯の下の線をもうちょっと上にあげる
    - [ ] vercel にあげてスマホで確認
- [x] コメントをつける (oxlint も確認する)
- [x] 警告を対応する
- [ ] React Compiler の準備
    - [x] useMemo や useCallback の禁止
    - [x] react-hooks の関連ルール導入
- [ ] 全体的にリファクタリングできる部分がないか検討する
- [ ] convex または Supabase につなぐ
    - [x] アカウント作成
- [ ] アプリ全体の設計
