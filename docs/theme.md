...
| ghost-fill     | 未登録の塗り | `oklch(0.96 0.008 345)` |
| ghost-stroke   | 未登録の輪郭 | `oklch(0.78 0.02 330)`  |
...
| ghost-fill     | 未登録の塗り | `oklch(0.25 0.02 300)`       |
| ghost-stroke   | 未登録の輪郭 | `oklch(0.45 0.03 325)`       |

`ghost-fill` と `ghost-stroke` は、アイテムが 0 点のカテゴリを示すゴースト専用の色
（`wireframes.md` の「空のカテゴリは『未登録』と『なし』を分ける」を参照）。
**濃すぎると登録済みのアイテムに見えてしまう**ため、地よりわずかに沈む程度にとどめる。
3 段階を並べて比較し、いちばん薄い案を採った。ダーク側は明暗を反転させた導出値で、
実物での確認はまだ済んでいない。
